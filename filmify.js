...
(function () {
  ...
  /* ---------------------------------------------- puerta de acceso
     Solo la home pide sesión: si no hay ninguna, se manda a la pantalla de
     acceso. «Recordar sesión» la guarda entre visitas; si no, dura solo la
     pestaña actual. El resto de páginas quedan accesibles por enlace directo. */
  (function puerta() {
    const p = location.pathname;
    const esHome = /(^|\/)index\.html?$/.test(p) || /\/$/.test(p);
    if (!esHome) return;
    let logged = false;
    try {
      logged = sessionStorage.getItem('filmify:sesion') ||
               localStorage.getItem('filmify:recordar');
    } catch (e) { logged = true; } // si el navegador bloquea storage, no molestamos
    if (!logged) location.replace('entrar.html');
  })();
