...
window.FILMIFY = (function () {
  ...
  /* Seguro de daños incluido en TODO alquiler. Es una póliza colectiva:
     Filmify es el tomador y cada reserva emite un certificado individual
     a nombre del inquilino, con la prima ya incluida en el precio.
     No hay fianza. El inquilino solo asume la franquicia si hay siniestro. */
  const SEGURO = {
    incluido: true,
    cubre: ['Daños por accidente', 'Robo con violencia', 'Expoliación'],
    limite: 40000,          // máximo asegurable por equipo o kit
    limiteEspecial: 50000,  // hasta aquí con aceptación especial
    franquiciaPct: 0.10,    // 10 % del coste del daño...
    franquiciaMin: 80,      // ...con un mínimo de 80 €
    unSiniestro: true,      // un siniestro cubierto por período de alquiler
    ambito: 'España y viajes durante el alquiler',
    excluye: [
      'Robo sin violencia y hurto',
      'Pérdida o extravío del equipo',
      'Accesorios y consumibles (cargadores, cables, manos libres)',
      'Software, datos y contenido grabado',
      'Daños estéticos que no impiden el uso',
      'Desgaste por uso normal',
      'Negligencia grave o daño intencionado'
    ],
    depreciacion: [
      { hasta: 3,  texto: 'Hasta 3 años, valor a nuevo' },
      { hasta: 10, texto: 'Desde el 4.º año, −10 % del valor de compra por año' },
      { hasta: 99, texto: 'Pasados 10 años, valor de segunda mano' }
    ]
  };
  ...
  /* Política de cancelación única y fija para toda la plataforma. */
  const POLITICA = {
    nombre: 'Flexible',
    texto: 'Devolución del 100 % hasta 24 h antes del inicio. Dentro de las últimas 24 h, se devuelve el 50 %.'
  };
  ...
  /* Franquicia de un siniestro: 10 % del coste del daño, con un mínimo
     de 80 €. Es lo único que puede pagar el inquilino, y solo si rompe algo.
     No hay fianza retenida. */
  const franquicia = coste =>
    Math.max(SEGURO.franquiciaMin, Math.round((coste || 0) * SEGURO.franquiciaPct));

  /* Valor indemnizable según la antigüedad del equipo (tabla del seguro):
     valor a nuevo hasta 3 años, −10 % del valor de compra por año a partir
     del 4.º, y valor residual pasados los 10. */
  const valorIndemnizable = (valor, anios) => {
    if (!anios || anios <= 3) return valor;
    if (anios <= 10) return Math.round(valor * (1 - 0.10 * (anios - 3)) / 10) * 10;
    return Math.round(valor * 0.30 / 10) * 10;
  };

  /* El seguro de daños va incluido y no suma al total: el inquilino paga
     alquiler, envío y comisión. Sin depósito. */
  const desglose = (eq, nDias, entrega) => {
    ...
    return {
      ...
      alquiler, envio, comision,
      total: alquiler + envio + comision
    };
  };

  return {
    CATS, CIUDADES, DUENOS, EQUIPOS, RESERVAS, ESTADOS,
    SEGURO, ENVIOS, POLITICA, GRAD, COMISION_INQUILINO,
    cat, dueno, equipo, eur, num, dias, fechaCorta,
    precioAlquiler, franquicia, valorIndemnizable, desglose
  };
})();
